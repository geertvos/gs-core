		...
		while( !threads.isEmpty() )
		{
			...
			while(threadIterator.hasNext()) {
				...
				case INVOKE:
					{
						...
						thread.setBytecode(functionDescription.getBytecode());
				...
				case RETURN:
					{
						...
						thread.setBytecode(program.getFunction(thread.getFunctionPointer()).getBytecode());
						thread.getBytecode().seek(pc);
						thread.getStack().push(v);
						gc.collect(heap, thread.getStack());
					}
					break;
				case PUT:
					{
				...
				case GETDYNAMIC:
				{
					...
					if(theValue == null) {
						GVMObject vo = heap.get(thread.getCallStack().peek().getScope().getValue());
				...
				case NATIVE: {
					...
					this.gc.collect(heap, thread.getStack());
					break;
				}	
				case THROW: {
					Value arg = thread.getStack().pop();
